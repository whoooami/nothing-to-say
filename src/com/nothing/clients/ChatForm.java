...
//	public JButton btnMini = null;
	...
	public ChatForm(String sendID,String recvID) {
		...
		this.setTitle(sendID+" 和 "+recvID+" 聊天中");
		super.lbTitle.setText(sendID+" 和 "+recvID+" 聊天中");
	...
	public class ExitEvt implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			/*if(vc!=null && vc.getState().equals(State.RUNNABLE)){
				...
				int option = JOptionPane.showOptionDialog(ManageChatForm.getChatFormByID(cfid), "正在进行语单通讯，是否要关闭？", "提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, null);
				if(option == JOptionPane.YES_OPTION){
					vc.interrupt();
					if(vc.isInterrupted()){
						System.out.println("语音线程结束成功!");
					}
				}else{
