		...
		if(tabbedPane.getSelectedIndex() == 0 ){
			if(allRBtn.isSelected() == true){
				...
				if(uid.equals("") && unickname.equals("")){
					Tools.alert(SearchConditionForm.this, "请输入查询条件.");
				}else{
					if(!uid.equals("") && !unickname.equals("")){
					...
					if(users.size()>0){
						...
						Tools.alert(SearchConditionForm.this, "未找到用户!");
					}
				}
			}
		}else{
			...
			if(groupNum_TF.getText().equals("")){
				Tools.alert(SearchConditionForm.this, "请输入群号码");
			}else{
				...
				if(group.size() > 0){
					...
					Tools.alert(SearchConditionForm.this, "未找到群.");
				}
			}
		}
	}
