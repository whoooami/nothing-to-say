	...
	public FindPwdForm() {
		...
		findPwdBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				if(uID.trim().equals("")){
					Tools.alert(FindPwdForm.this, "用户ID不能为空！");
				}else if(uMail.trim().equals("")){
					Tools.alert(FindPwdForm.this, "电子邮件不能为空");
				}else{
					...
					if(l.size()>0){
						...
						Tools.alert(FindPwdForm.this, "找回成功，密码为："+u.getuPassword());
						dispose();
						LoginIN login = new LoginIN();
						login.setLocationRelativeTo(null);
						login.setVisible(true);
					}else{
						Tools.alert(FindPwdForm.this, "别忽悠了，这账号你小子地吗？");
					}
				}
			}
		});
