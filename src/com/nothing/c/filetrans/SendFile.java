...
//			ManageChatForm.operaFTCount(cfid, "+");
			Tools.addMessage(ManageChatForm.getChatFormByID(cfid).tpRecv, "对方已经开始接收文件！");
			int tmp = 0;
			while(true){
				tmp = bis.read();
				if(-1 == tmp){
			...
			Tools.addMessage(ManageChatForm.getChatFormByID(cfid).tpRecv, file.getName()+"文件发送完毕！");
			bis.close();
			bos.flush();
			bos.flush();
			s.close();
...
//			ManageChatForm.operaFTCount(cfid, "-");
	...
	public void chooseSendFile(){
		...
		int option = fileChooser.showDialog(null, "选择传输的文件");
